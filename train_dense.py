...
from model.Resnet_base_50_model import Unet_Resnet
from model.tiramisu import FCDenseNet57
import logging
import torch
...
parser.add_argument('--model', type=str,help='the model you want to use, now we support unet, unetpp, unet2, unet3, resunet, denseunet', default="unet")
...
parser.add_argument('--start_epoch', type=int, help='training epoch start from', default=0)
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            if (i+1) % 1 == 0:
                print ("Epoch [{}/{}], Step [{}/{}] Loss: {:.4f}"
                    .format(epoch, end, i+1, total_step, loss.item()))
        try:
            os.mkdir(dir_checkpoint)
            logging.info('Created checkpoint directory')
        except OSError:
            pass
        torch.save(net.state_dict(),dir_checkpoint + '{}_CP_epoch{}.pth'.format(args.model, epoch))
...
for i, (images, labels) in enumerate(test_loader):
    ...
    loss = criterion(outputs, labels)
    print(loss.item())
    savepic(outputs[0,:], "./resnet_result/{}_predict_{}.png".format(i, args.model))